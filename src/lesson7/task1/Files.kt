...
fun countSubstrings(inputName: String, substrings: List<String>): Map<String, Int> {
    ...
    val str = File(inputName).bufferedReader().readLines().toString()
    for (i in substrings) {
        if (str.contains(("\\$i").toRegex(RegexOption.IGNORE_CASE))) {
            result[i] = ("(?=\\$i)").toRegex(RegexOption.IGNORE_CASE).findAll(str).count()
        }
    }
    return result
}
